...
contract TestHelper is Test, OptionsHelper {
        ...
        returns (address[] memory oapps)
    {
        ...
        for (uint8 eid = _startEid; eid < _startEid + _oappNum; eid++) {
            address oapp = _deployOApp(
                _oappCreationCode,
                abi.encodeWithSignature("initialize(address,address)", address(endpoints[eid]), address(this))
            );
    ...
    function _deployOApp(bytes memory _oappBytecode, bytes memory _initialCalldata) internal returns (address addr) {
        address impl;
        assembly {
            impl := create(0, add(_oappBytecode, 0x20), mload(_oappBytecode))
            if iszero(extcodesize(impl)) { revert(0, 0) }
        }
        addr = address(new ERC1967Proxy(impl, _initialCalldata));
    }
